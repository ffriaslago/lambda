...
(define sRam (weighted-pairs integers integers weightRam))

(weightRam (stream-ref sRam 0)) ; (1 1). W2

(weightRam (stream-ref sRam 50)) ; 1332

(weightRam (stream-ref sRam 60)) ; 1729
(stream-ref sRam 60) ; (1 12)
(weightRam (stream-ref sRam 61)) ; 1729
(stream-ref sRam 61) ; (9 10)


(define (stream-filterv2 pred stream) ; Alternative version to take two consecutive elements
  (cond ((stream-null? stream) 
         the-empty-stream)
        
        ((pred (stream-car stream) (stream-car (stream-cdr stream)))
         (cons-stream 
          (cons-stream (stream-car (stream-cdr stream)) (stream-car stream)) ; Change order to have the other pair
          (stream-filterv2 
           pred
           (stream-cdr stream))))
        
        (else (stream-filterv2 
               pred 
               (stream-cdr stream)))))
...
(define RamStream
  (stream-filterv2
   (lambda (x y)
     (= (weightRam x) (weightRam y)))
   sRam))

(stream-ref RamStream 0) ; (1 12) -> 1729 -> (9 10)
(stream-ref RamStream 1) ; (2 16) -> 4104 -> (9 15)
(stream-ref RamStream 2) ; (2 24) -> 13832 -> (18 20)
(stream-ref RamStream 3) ; (10 27) -> 20683 -> (19 24)
(stream-ref RamStream 4) ; (4 32) -> 32832 -> (18 30)
(stream-ref RamStream 5) ; (2 34) -> 39312 -> (15 33)

; After checking, it is the right sequence. Sadly, it is not able to give the second pair of numbers. If the order of stream-filterv2 is changed, maybe
